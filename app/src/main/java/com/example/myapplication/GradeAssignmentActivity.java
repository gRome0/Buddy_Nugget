...
public class GradeAssignmentActivity extends AppCompatActivity {
    private Spinner assignmentSpinner; private EditText studentIdField, scoreField; private Button gradeBtn;

    private Map<String, Integer> assignmentMap = new HashMap<>();

    @Override
    protected void onCreate(Bundle savedInstanceState) { super.onCreate(savedInstanceState); setContentView(R.layout.activity_grade_assignment);
        assignmentSpinner = findViewById(R.id.assignmentSpinner); studentIdField = findViewById(R.id.gradeStudentId);
        scoreField = findViewById(R.id.gradeScore); gradeBtn = findViewById(R.id.submitGradeBtn);
        ...
        gradeBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                {Toast.makeText(GradeAssignmentActivity.this, "All fields are required", Toast.LENGTH_SHORT).show(); return; }

                try { int assignmentId = assignmentMap.get(selectedLabel); float score = Float.parseFloat(scoreText);
    ...
    public static Intent newIntent(android.content.Context context) { return new Intent(context, GradeAssignmentActivity.class); }
}